...
| GAP-RUNNER-002 | Testing (`CliRunner`) | `charset` exists but is not used for decoding output. | Low | Done | Added charset decoding with `encoding_rs` and coverage. |
...
| GAP-DECORATORS-001 | Decorators | `make_pass_decorator(ensure=...)` behavior is not implemented. | Medium | By Design | Callbacks receive `&Context`; implementing `ensure` would require API-breaking mutability/interior mutability. |
| GAP-PARITY-001 | Parity coverage | Parity suites exist for phases 1–6; phases 7+ not yet represented as parity tests. | Low | By Design | Parity scope intentionally limited; remaining behavior covered by unit tests and expanded only when needed. |
| GAP-CI-001 | CI parity | CI runs `cargo test` on Linux/macOS/Windows; parity runner is not part of CI by default. | Low | Done | Added optional parity job via workflow_dispatch (Linux). |
