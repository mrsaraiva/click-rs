...
| GAP-PARAM-001 | Parameters | Type conversion is not applied during parsing; `Argument`/`ClickOption` values are stored as raw strings. | High | Open | Wire `TypeConverter` (and option converter) into `Command` parsing, including error handling. |
